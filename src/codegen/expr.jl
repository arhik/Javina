...
struct BinaryExpr <: BinaryOp
	...
	left::Union{Ref{JVVariable}, Scalar, JLExpr}
	right::Union{Ref{JVVariable}, Scalar, JLExpr}
end
...
struct CallExpr <: JLExpr
	func::Union{Ref{JVVariable}, JLExpr}
	args::Vector{Union{Ref{JVVariable}, Scalar, JLExpr}}
end
...
symbol(a::Vector{T}) where T <: Union{JVVariable, Scalar, JLExpr} = (map(symbol, a))
...
struct IndexExpr <: JLExpr
	sym::Union{Ref{JVVariable}, JLExpr}
	idx::Union{Ref{JVVariable}, Scalar, JLExpr}
end
...
struct AccessExpr<: JLExpr
	sym::Union{Ref{JVVariable}, JLExpr}
	field::Union{Ref{JVVariable}, JLExpr}
end
...
typeInfer(scope::Scope, tExpr::TypeExpr) = begin
	if symbol(tExpr)[1] == :Array # Hardcoded
...
typeInfer(scope::Scope, declexpr::DeclExpr) = begin
	...
	if found == false && location != :typeScope
		...
		error("duplicate declaration of a variable $sym is not allowed in JV though julia allows it")
	end
