...
makeVarPair(p::Pair{Symbol, DataType}) = JVVariable(p.first, p.second, Generic, nothing, false, false)
...
function computeBlock(scope, islaunch, wgSize, wgCount, shmem, funcName, funcArgs, fexpr::Expr)
	...
	for JVf in JVfunctions
	    scope.globals[JVf] = makeVarPair(JVf=>Function)
    end
		...
		elseif @capture(symbolarg, iovar_::ioType_)
			if eltype(inarg) <: Number
				push!(
					scope.code.args,
					quote
						@const $iovar::$(eltype(inarg)) = $(Meta.parse((JVType(inarg))))
					end
				)
	...
	for (idx, (inarg, symbolarg)) in enumerate(zip(funcArgs, fargs))
		if @capture(symbolarg, iovar_::ioType_{T_, N_})
			# TODO instead of assert we should branch for each case of argument
			@assert ioType == :Array # "Expecting Array Type, received $ioType instead"
	...
	for (i, arg) in enumerate(fa)
		...
		(arg.sym[]).varAttr = JVVariableAttribute(0, bindingCount)
		bindingCount += 1
	end
