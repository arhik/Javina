...
function inferExpr(scope::Scope, expr::Expr)
	...
	elseif @capture(expr, @JVatomic aExpr_)
		return atomicExpr(scope, aExpr)
	elseif 	@capture(expr, @JVkernel islaunch_ workgroupSize_ workgroupCount_ function fname_(fargs__) where Targs__ fbody__ end)
		return computeBlock(scope, islaunch, workgroupSize, workgroupCount, fname, fargs, Targs, fbody)
	elseif 	@capture(expr, @JVkernel islaunch_ workgroupSize_ workgroupCount_ fname_(fargs__))
...
function inferExpr(scope::Scope, a::Symbol)
	...
	var = Ref{JVVariable}()
	if found == false
		var[] = JVVariable(a, Any, Generic, nothing, false, true)
		scope.globals[a] = var[]
...
function inferExpr(scope::Scope, a::JVScalarType)
	return Scalar(a)
end
